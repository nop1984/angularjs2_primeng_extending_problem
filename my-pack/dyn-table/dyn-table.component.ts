...
import {PaginatorModule} from 'primeng/components/paginator/paginator'; //'../paginator/paginator';
import {Column,Header,Footer,HeaderColumnGroup,FooterColumnGroup,PrimeTemplate} from 'primeng/components/common/shared';// '../common/shared';
import {LazyLoadEvent} from 'primeng/components/common/lazyloadevent'; //'../common/lazyloadevent';
import {FilterMetadata} from 'primeng/components/common/filtermetadata'; //'../common/filtermetadata';
import {SortMeta} from 'primeng/components/common/sortmeta'; //'../common/sortmeta';
import {DomHandler} from 'primeng/components/dom/domhandler'; //'../dom/domhandler';
import {ObjectUtils} from 'primeng/components/utils/objectutils'; //'../utils/objectutils';
import {Subscription} from 'rxjs/Subscription';
import {BlockableUI} from 'primeng/components/common/blockableui'; //'../common/blockableui';

import {  HttpResponse, HttpRequest,  HttpEvent,  HttpEventType, HttpErrorResponse, HttpClient } from '@angular/common/http';
...
export class ScrollableView implements AfterViewInit,AfterViewChecked,OnDestroy {
    ...
    constructor(@Inject(forwardRef(() => DynTable)) public dt:DynTable, public domHandler: DomHandler, public el: ElementRef, public renderer: Renderer2, public zone: NgZone) {}
...
export class DynTable implements AfterViewChecked,AfterViewInit,AfterContentInit,OnInit,OnDestroy,BlockableUI {
    ...
    set value(val:any[]) {
        console.log('*** Parent setter ( this.immutable )*** ' + this.immutable);
        //source.map(x => Object.assign({}, x));
        if(this.immutable) {
            //this._value = val ? [...val] : null;
            this._value = val.map(x => Object.assign({}, x));
...
//    exports: [DynTable,SharedModule],
//    declarations: [DynTable,DTRadioButton,DTCheckbox,ColumnHeaders,ColumnFooters,TableBody,ScrollableView,RowExpansionLoader]
//})
